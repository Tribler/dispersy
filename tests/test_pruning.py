...
class TestPruning(DispersyTestFunc):
    ...
    def _create_prune(self, node, globaltime_start, globaltime_end, store=True):
        messages = [node.create_full_sync_global_time_pruning_text("Hello World #%d" % i, i) for i in range(globaltime_start, globaltime_end + 1)]
        if store:
            node.store(messages)
        return messages

    def _create_normal(self, node, globaltime_start, globaltime_end, store=True):
        messages = [node.create_full_sync_text("Hello World #%d" % i, i) for i in range(globaltime_start, globaltime_end + 1)]
        if store:
            node.store(messages)
        return messages
    ...
    def test_local_creation_causes_pruning(self):
        ...
        # check settings
        meta = self._community.get_meta_message("full-sync-global-time-pruning-text")
    ...
    def test_remote_creation_causes_pruning(self):
        ...
        messages = other.fetch_messages(["full-sync-global-time-pruning-text", ])
        self.assertTrue(all(message.distribution.pruning.is_active() for message in messages), "all messages should be active")
    ...
    def test_sync_response_response_filtering_inactive(self):
        ...
        other.give_message(node.create_introduction_request(other.my_candidate, node.lan_address, node.wan_address, False, "unknown", sync, 42, global_time), node)

        # OTHER should return the 10 active messages
        responses = [response for _, response in node.receive_messages(names=["full-sync-global-time-pruning-text"])]
